...
async def summarize_conversation(state: AgentState) -> SummaryReturn:
    ...
    summary_msg = SystemMessage(content=f"Previous conversation summary: {response.content}")
...
async def synthesize_node(state: AgentState) -> SynthesisReturn:
    ...
    route = state.get("routing_decision", "").split("[Selected Route]")[1].split("\n")[1].strip().upper()
    
    # Case 1: Coming from RAG graph
    if route == "RAG" and (rag_response := state.get("rag_response")):
        source = rag_response.get("source")
        
        if source == "process_document" and (document_status := rag_response.get("document_status")):
            return {
                "messages": [AIMessage(content=f"""Successfully processed document: {document_status['metadata'].get('source', 'document')}
...
{rag_analysis.replace("[Answer]", "").replace("[Sources]", "\nBased on:").strip()}"""
    ...
    if route == "RESEARCH" and (research_response := state.get("research_response")):
        if research_response.get("source") == "researcher":
            return {
                "messages": [AIMessage(content=f"""Here are the findings from my research:
...
{research_response["research_findings"]}""")]
    ...
    return {
        "messages": [AIMessage(content="I encountered an unexpected state and cannot provide a proper response.")]
    }
...
async def route_message(state: AgentState) -> RouterReturn:
    ...
    response = await model.ainvoke([
        ...
        HumanMessage(content=messages[-1].content)  # Route based on last message with context
    ])
    
    # Get the route from the response
    next_step = response.content.split("[Selected Route]")[1].split("\n")[1].strip().upper()
    ...
    return {
        ...
        "next": route_mapping.get(next_step, "research")  # Default to research if unknown route
    }
...
def route_condition(state: AgentState) -> str:
    ...
    try:
        route = routing_decision.split("[Selected Route]")[1].split("\n")[1].strip().upper()
        route_mapping = {
            "ANSWER": "answer",
            "RESEARCH": "research",
            "RAG": "rag"
