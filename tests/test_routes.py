...
from unittest.mock import (
    ...
    AsyncMock,
    ANY,
)  # Ensure AsyncMock and ANY are imported
...
async def test_upload_txt_file(async_client: AsyncClient):
    ...
    with patch(
        "src.backend.routes.ingest_file", new_callable=MagicMock
    ) as mock_ingest_file:
...
async def test_upload_ingestion_general_error(async_client: AsyncClient):
    ...
    with patch(
        ...
        mock_ingest_file.return_value = {
            "status": "error",
            "error": "Some generic ingestion failure",
        }
        response = await async_client.post("/api/upload-document/", files=files)
...
async def test_get_conversations_list_success(async_client: AsyncClient):
    ...
    with patch(
        "src.backend.routes.langgraph_client.threads.search", new_callable=AsyncMock
    ) as mock_search:
...
async def test_get_conversations_list_client_error(async_client: AsyncClient):
    ...
    assert (
        "Failed to fetch conversations: LangGraph client error"
        in json_response["error"]["detail"]
    )
    mock_search.assert_called_once_with(limit=1000)


async def test_get_conversation_by_id_success_and_message_parsing(
    ...
    with patch(
        "src.backend.routes.langgraph_client.threads.create", new_callable=AsyncMock
    ) as mock_create, patch(
        "src.backend.routes.langgraph_client.threads.get_state", new_callable=AsyncMock
    ) as mock_get_state:
    ...
    mock_create.assert_called_once_with(
        thread_id=thread_id, if_exists="do_nothing", metadata=ANY
    )
...
async def test_delete_all_threads_success(async_client: AsyncClient):
    ...
    with patch(
        ...
        "src.backend.routes.langgraph_client.threads.delete", new_callable=AsyncMock
    ) as mock_delete:
    ...
    assert json_response == {
        "success": True,
        "message": f"Successfully deleted {len(mock_threads_found)} threads",
    }
...
async def test_delete_all_threads_no_threads_exist(async_client: AsyncClient):
    ...
    assert json_response == {
        "success": True,
        "message": "Successfully deleted 0 threads",
    }
...
async def test_delete_all_threads_search_fails(async_client: AsyncClient):
    ...
    )  # ConflictError from the route's exception handling
    json_response = response.json()
    assert json_response["error"]["code"] == "BULK_DELETE_ERROR"
    assert (
        "Failed to delete all threads: LangGraph client error during search"
        in json_response["error"]["detail"]
    )
...
async def test_delete_all_threads_one_delete_fails(async_client: AsyncClient):
    ...
    )  # Endpoint itself succeeds even if some individual deletes fail
    json_response = response.json()
    # The message reflects the count of *successful* deletions based on current routes.py logic
    assert json_response == {
        "success": True,
        "message": f"Successfully deleted {expected_deleted_count} threads",
    }

    mock_search.assert_called_once_with(limit=1000)
    assert mock_delete.call_count == len(
        mock_threads_found
    )  # Delete is attempted for all
...
async def test_delete_thread_not_found_explicit_assert_not_called(
    async_client: AsyncClient,
):
...
async def test_stream_message_success_and_logic(async_client: AsyncClient):
    ...
    async def mock_join_stream_generator(*args, **kwargs):
        ...
        yield {
            ...
            "data": [
                {
                    "langgraph_node": "some_previous_node",
                    "created_by": "system",
                },  # Node transition
                {
                    "type": "AIMessageChunk",
                    "content": "This should be ignored (not answer node).",
                    "langgraph_node": "some_previous_node",
                },
            ],
        }
    ...
    with patch(
        "src.backend.routes.langgraph_client.runs.join_stream",
        new=mock_join_stream_generator,
    ) as mock_join_stream:
        response = await async_client.get(
            f"/api/conversations/{thread_id}/stream-message?run_id={run_id}"
        )
        assert response.status_code == 200
    ...
    expected_events = [
        ...
        {
            "event": "message",
            "data": {"role": "assistant", "content": "Calculation done."},
        },
    ...
    mock_join_stream.assert_called_once_with(
...
async def test_send_message_success(async_client: AsyncClient):
    ...
    expected_langgraph_input = {
        "messages": [{"type": "human", "content": "Hello, is anyone there?"}]
    }

    with patch(
        "src.backend.routes.langgraph_client.runs.create", new_callable=AsyncMock
    ) as mock_runs_create:
        mock_runs_create.return_value = {
            "run_id": run_id
        }  # Simulate successful run creation

        response = await async_client.post(
            f"/api/conversations/{thread_id}/send-message", json=request_payload
        )
...
async def test_send_message_thread_not_found_or_error(async_client: AsyncClient):
    ...
    with patch(
        ...
        mock_runs_create.side_effect = Exception(
            "LangGraph client error: Thread not found or other issue"
        )
    ...
    )  # NotFoundError from the route's exception handling
    json_response = response.json()
    assert (
        json_response["error"]["code"] == "THREAD_NOT_FOUND"
    )  # Code used in the route
    assert (
        f"Thread {thread_id} not found or error creating run"
        in json_response["error"]["detail"]
    )
...
async def test_get_conversation_by_id_get_state_error(async_client: AsyncClient):
    ...
    with patch(
        ...
        mock_get_state.side_effect = Exception(
            "LangGraph client error during get_state"
        )
